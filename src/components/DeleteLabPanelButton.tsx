...
import { deleteLabPanel } from "@/app/actions/bloodwork";

export function DeleteLabPanelButton({ id, label }: { id: string; label: string }) {
  ...
  if (!confirming) {
    return (
      <span className="flex items-center gap-2 text-xs">
        <button type="button" onClick={() => setConfirming(true)} aria-label={`Delete lab panel ${label}`} className="inline-flex items-center gap-1 font-medium text-muted hover:text-danger"><Trash2 className="h-3.5 w-3.5" aria-hidden /> Delete</button>
