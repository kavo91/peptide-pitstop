 ...
 *   - pitstop: telemetry stat tile — leads with the big coverage-days number
 *     (mono, with a small muted "d"), peptide name as a sub-line, an orange
 *     "Reorder" chip pinned to the bottom when urgent, and a 3px bottom accent
...
export function SupplyTile({ item, design }: Props) {
  const pit = design === "pitstop";

  if (!item) {
    // Empty state — shared by both designs (unchanged).
    return (
