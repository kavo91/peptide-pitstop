...
const DEVICE_TZ = "America/New_York"; // UTC−4 in July → 14 h behind the runtime zone
...
describe("slotTimeLabel", () => {
  it("adds the device-local time when the zones differ", () => {
    expect(slotTimeLabel(AT_2100, "21:00", DEVICE_TZ)).toBe("21:00 (07:00 your time)");
  });
...
describe("reminder bodies", () => {
  it("slot reminder states both clocks while travelling", () => {
    const [ev] = buildReminderEvents([dose()], AT_2100, 30, { viewerTz: DEVICE_TZ });
  ...
  it("nag labels use the device clock alone, not a nested double time", () => {
    // 08:00 runtime-local = 18:00 the previous day on the device. The dose's moment
  ...
  it("timing is untouched — the event still fires on the home-anchored moment", () => {
    ...
    expect(buildReminderEvents([dose()], new Date("2026-07-28T07:00:00"), 30, { viewerTz: DEVICE_TZ })).toHaveLength(0);
  });
});
